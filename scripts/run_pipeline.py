...
from src.data.charger_donne import load_data                    # Data loading with error handling
from src.data.preprocess import preprocess_data            # Basic data cleaning
from src.features.build_feature import build_features     # Feature engineering (CRITICAL for model performance)
...
def main(args):
    ...
    # Force un chemin relatif simple pour éviter les conflits de lettre de lecteur (C:)
    mlruns_path = "mlruns" 
    ...
    with mlflow.start_run():
        ...
        print("💾 Saving model (Direct + MLflow Tracking)...")

        # 1. On le garde dans MLflow pour l'historique (même si le dossier est dur à trouver)
        mlflow.sklearn.log_model(model, artifact_path="model")

        # 2. On le sauve en DIRECT dans ton dossier artifacts racine pour FastAPI
        # C'est cette ligne qui va créer le fichier que tu peux VOIR
        model_path = os.path.join(artifacts_dir, "model.json")
        model.save_model(model_path) 

        print(f"✅ Fichier créé : {model_path}")
